...
calibration_parameters_default = Dict( # The variables we wish to calibrate 
    ...
    "turbulent_entrainment_factor" => Dict(
        "prior_mean" => FT(default_params["turbulent_entrainment_factor"]["value"]),
        "constraints" => bounded(0.0, 10.0),
        "l2_reg" => 5.0 / 60.0,
        "CLIMAParameters_longname" => "",
    ),
    # Diffusion Parameters
    "tke_ed_coeff" => Dict(
        "prior_mean" => FT(default_params["tke_ed_coeff"]["value"]),
        "constraints" => bounded(0.01, 1.0),
        "l2_reg" => 5.0 / 60.0,
        "CLIMAParameters_longname" => "",
    ),
    "tke_diss_coeff" => Dict(
        "prior_mean" => FT(default_params["tke_diss_coeff"]["value"]),
        "constraints" => bounded(0.01, 1.0),
        "l2_reg" => 5.0 / 60.0,
        "CLIMAParameters_longname" => "",
    ),
    "static_stab_coeff" => Dict(
        "prior_mean" => FT(default_params["static_stab_coeff"]["value"]),
        "constraints" => bounded(0.01, 1.0),
        "l2_reg" => 5.0 / 60.0,
        "CLIMAParameters_longname" => "",
    ),
    "tke_surf_scale" => Dict(
        "prior_mean" => FT(default_params["tke_surf_scale"]["value"]),
        "constraints" => bounded(1.0, 16.0),
        "l2_reg" => 5.0 / 60.0,
        "CLIMAParameters_longname" => "",
    ),
    "Prandtl_number_0" => Dict(
        "prior_mean" => FT(default_params["Prandtl_number_0"]["value"]),
        "constraints" => bounded(0.5, 1.5),
        "l2_reg" => 5.0 / 60.0,
        "CLIMAParameters_longname" => "",
    ),
...
function get_reference_config(::SOCRATES_Train)
    ...
    config["Σ_t_start"] = repeat([0.5 * 3600], n_repeat) #  shorter for testing
...
function get_reference_config(::SOCRATES_Val)
    ...
    # config["batch_size"] = n_repeat # has to be some divisor of n_repeat, default is n_repeat == length(ref_dirs) == number of setups
    local_namelist = [
        ("thermodynamics", "moisture_model", "equilibrium"), # TC.jl || overwrite_namelist
