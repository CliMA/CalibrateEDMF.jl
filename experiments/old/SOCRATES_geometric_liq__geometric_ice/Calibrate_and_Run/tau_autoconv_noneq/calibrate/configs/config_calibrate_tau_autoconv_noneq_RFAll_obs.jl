...
ρ_w = 1000. # density of ice, default from ClimaParameters
...
calibration_parameters_default = Dict( # The variables we wish to calibrate , these aren't in the namelist so we gotta add them to the local namelist...
    #
    "geometric_liq_c_1"   => Dict("prior_mean" => FT(1/(4/3 * π * ρ_w * r_0^2))    , "constraints" => bounded_below(0) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing), 
    "geometric_liq_c_2"   => Dict("prior_mean" => FT(2/3.)                         , "constraints" => bounded(1/3., 1) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing), 
    "geometric_ice_c_1"   => Dict("prior_mean" => FT(1/(4/3 * π * ρ_w * r_0^2))    , "constraints" => bounded_below(0) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing), 
...
if calibrate_to == "Flight_Observations"
    if !ismissing(t_bnds.obs_data)
        ...
        _, obs_paths = process_SOCRATES_Flight_Observations_reference(out_vars=calibration_vars, fake_time_bnds = t_bnds.obs_data, out_dir=joinpath(experiment_dir, "Reference","Flight_Observations"), overwrite=false) # create trimmed data for calibration so the covarainces aren't NaN (failed anyway cause i think after interpolation NaNs can return because the non-NaN data don't form a contiguous rectangle :/ )
    end
    if !ismissing(t_bnds.ERA5_data)
        @info("Creating trimmed observational ERA5_data-forced data for calibration")
        _, ERA5_paths = process_SOCRATES_Flight_Observations_reference(out_vars=calibration_vars, fake_time_bnds = t_bnds.ERA5_data, out_dir=joinpath(experiment_dir, "Reference","Flight_Observations"), overwrite=false) # create trimmed data for calibration so the covarainces aren't NaN (failed anyway cause i think after interpolation NaNs can return because the non-NaN data don't form a contiguous rectangle :/ )
    end
    # concatenate
...
function get_output_config()
    config = Dict()
    config["outdir_root"] = joinpath(experiment_dir, "Calibrate_and_Run","tau_autoconv_noneq", "calibrate", "output", calibrate_to, "RFAll_obs") # store them in the experiment folder here by default
