...
this_dir = "/home/jbenjami/Research_Schneider/CliMa/CalibrateEDMF.jl/experiments/SOCRATES_Calibrate_exponential_T_scaling/"
...
function process_SOCRATES_Atlas_LES_reference(;
    ...
    out_dir::String = joinpath(this_dir, "Reference", "Atlas_LES"), # the folder where we store our truth (Atlas LES Data)
    overwrite::Bool = true,
)
    truth_files = filter(contains(".nc"), readdir(out_dir))
        ...
        for flight_number in flight_numbers
            ...
            if !isempty(truth_file) # if we have a file
                ...
                if !isfile(outpath) || overwrite # only do if we don't have the file or we want to overwrite
                    ...
                    @info(
                        "Skipping existing file for flight $flight_number and forcing $forcing_type since overwrite is false"
                    )
                end
            else
                @info(
                    "Skipping creating reference due to missing truth file for flight $flight_number and forcing $forcing_type"
                )
            end
        end
    end
