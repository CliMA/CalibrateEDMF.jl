...
calibrated_config_path = joinpath(experiment_dir, "Calibrate_and_Run", this_calibrate_and_run , "calibrate", "output", calibrated_config_dir)
...
for (i,parameters) in enumerate(parameters_list) # store things so we can iterate jobs regardless of which parameters dict theyre from, so that we can keep all workings going all the time (since the parameters dicts ave different numbers of jobs)
    ...
    output_dir = joinpath(experiment_dir, "Calibrate_and_Run", this_calibrate_and_run, "run", "output", calibrated_config_dir, datetime_suffixes[i])  # drop the datetime info, just keep the file at the suffix so if we run again it just overwrites...
    mkpath(output_dir) # make it now so we only have to do it once...
    cp(@__FILE__, joinpath(output_dir, "run_calibrated_"*this_calibrate_and_run*"_RF"*flight_str*"_obs.jl"), force=true)
