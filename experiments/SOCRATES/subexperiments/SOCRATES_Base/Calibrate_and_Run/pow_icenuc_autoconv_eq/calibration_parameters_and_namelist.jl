...
calibration_parameters__experiment_setup = Dict( # The variables we wish to calibrate , these aren't in the namelist so we gotta add them to the local namelist...
    #
    "pow_icenuc" => Dict(
        "prior_mean" => global_param_defaults["pow_icenuc"]["prior_mean"],
        "constraints" => global_param_defaults["pow_icenuc"]["constraints"],
        "l2_reg" => nothing,
        "CLIMAParameters_longname" => "pow_icenuc",
        "unconstrained_σ" => global_param_defaults["pow_icenuc"]["unconstrained_σ"],
    ),
    #
