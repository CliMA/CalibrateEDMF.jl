...
calibration_parameters__experiment_setup = Dict( # The variables we wish to calibrate , these aren't in the namelist so we gotta add them to the local namelist...
    ...
    #
    "τ_sub_dep" => Dict(
        "prior_mean" => global_param_defaults["τ_sub_dep"]["prior_mean"],
        "constraints" => global_param_defaults["τ_sub_dep"]["constraints"],
        "l2_reg" => nothing,
        "CLIMAParameters_longname" => "sublimation_deposition_timescale",
        "unconstrained_σ" => global_param_defaults["τ_sub_dep"]["unconstrained_σ"],
    ),
)
