...
calibration_parameters_default = Dict( # The variables we wish to calibrate , these aren't in the namelist so we gotta add them to the local namelist...
    #
    "geometric_liq_c_1"   => Dict("prior_mean" => FT(1/(4/3 * π * ρ_l * r_r^2))    , "constraints" => bounded_below(0) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing), # Inhomogenous default assuming r_0 = 20 micron since `typical` N is harder to define
    "geometric_liq_c_2"   => Dict("prior_mean" => FT(2/3.)                         , "constraints" => bounded(1/3., 1) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing), # Halfway between 1/3 and 1
    "geometric_liq_c_3"   => Dict("prior_mean" => FT(N_l * r_0)                    , "constraints" => bounded_below(0) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing), 
    #
    "geometric_and_T_scaling_ice_c_1"   => Dict("prior_mean" => FT((4*π*D) * ((4/3 * π * ρ_i)^(-1/3)  * (N_i)^(2/3) * (0.02)^(2/3) + (N_i * r_0))) , "constraints" => bounded_below(0) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing), # Yeahhh.... idk for this one lol... just combined them serially from the homogenous case where c_3 is -1/3
    "geometric_and_T_scaling_ice_c_2"   => Dict("prior_mean" => FT(2/3.)                                                                               , "constraints" => bounded(1/3., 1) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing),# Halfway between 1/3 and 1 -- should this be the same as c_2g? It's the same mixing...
    "geometric_and_T_scaling_ice_c_3"   => Dict("prior_mean" => FT((4*π*D) * r_0 * .02 )                                                           , "constraints" => bounded_below(0) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing), # Fletcher 1962 (values taken from Frostenberg 2022)
    "geometric_and_T_scaling_ice_c_4"   => Dict("prior_mean" => FT(-0.6)                                                                               , "constraints" => bounded_above(0) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing), # Fletcher 1962 (values taken from Frostenberg 2022)
    #
