...
supersat_type = :exponential_T_scaling_ice_raw
...
calibration_parameters_default = Dict( # The variables we wish to calibrate , these aren't in the namelist so we gotta add them to the local namelist...
    ...
    #
    "T_scaling_ice_c_1"   => Dict("prior_mean" => FT(4*π*D * (q_i_0 / (4/3) * π * ρ_i)^(1/3) * (0.02)^(2/3))     , "constraints" => bounded_below(0) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing, "unconstrained_σ" => bounded_edge_σ), 
    "T_scaling_ice_c_2"   => Dict("prior_mean" => FT(-0.6 * 2/3)    , "constraints" => bounded_above(0) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing, "unconstrained_σ" => bounded_edge_σ), 
    #
