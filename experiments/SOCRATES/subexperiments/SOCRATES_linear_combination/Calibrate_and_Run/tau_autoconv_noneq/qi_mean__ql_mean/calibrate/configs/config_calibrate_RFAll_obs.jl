...
supersat_type = :linear_combination
...
calibration_parameters_default = Dict( # The variables we wish to calibrate , these aren't in the namelist so we gotta add them to the local namelist... 
    #
    "linear_combination_liq_c_1"   => Dict("prior_mean" => FT(N_l * r_0)                     , "constraints" => bounded_below(0) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing, "unconstrained_σ" => bounded_edge_σ), # I think at q=0, we need c_1 from linear = c_1 from geometric...
    "linear_combination_liq_c_2"   => Dict("prior_mean" => FT(0)                             , "constraints" => no_constraint()  , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing, "unconstrained_σ" => expanded_unbounded_σ), # T scaling on liq uncertain
    "linear_combination_liq_c_3"   => Dict("prior_mean" => FT(1)                             , "constraints" => no_constraint()  , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing, "unconstrained_σ" => expanded_unbounded_σ), # should have τ down as q up, so start positive
    #
    "linear_combination_ice_c_1"   => Dict("prior_mean" => FT(N_i * r_0)                     , "constraints" => bounded_below(0) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing, "unconstrained_σ" => bounded_edge_σ), # I think at q=0, we need c_1 from linear = c_1 from geometric...
    "linear_combination_ice_c_2"   => Dict("prior_mean" => FT(-0.6)                          , "constraints" => bounded_above(0) , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing, "unconstrained_σ" => bounded_edge_σ), # Fletcher 1962 (values taken from Frostenberg 2022), same sign again I suppose...
    "linear_combination_ice_c_3"   => Dict("prior_mean" => FT(1)                             , "constraints" => no_constraint()  , "l2_reg" => nothing, "CLIMAParameters_longname" => nothing, "unconstrained_σ" => expanded_unbounded_σ), # should have τ down as q up, so start positive
    #
...
local_namelist = [ # things in namelist that otherwise wouldn't be... (both random parameters and parameters we want to calibrate that we added ourselves that generate_namelist doesn't insert...)
    ...
    # ("user_aux", "linear_combination_liq_c_4", calibration_parameters["linear_combination_liq_c_4"]["prior_mean"] ), # do we need an offset?
    #
