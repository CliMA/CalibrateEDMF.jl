...
#SBATCH -J "launch_calibrate_tau_autoconv_eq"   # job name
#SBATCH --mem-per-cpu=1G   # memory per CPU core
#SBATCH --output=/dev/null

# TO-DO -- add command line option to specify flights we want as comma-separated list, and set default to just be "All" only, and then switch below to looping over the flights
experiment=${1?Error: no experiment given}
calibration_setups=${2?Error: no calibration setup given} # the subname of the calibration setup within the experiment (these should almost just be different experiments tbh)

calibrate_script=~/Research_Schneider/CliMa/CalibrateEDMF.jl/experiments/SOCRATES/julia_parallel/calibrate_script # update these to work form anywhere so can use HPC too... see start_calibration.sh
...
# if [ -n "$SLURM_JOB_ID" ];  then # need to add quotes here, see https://unix.stackexchange.com/questions/109625/shell-scripting-z-and-n-options-with-if#comment1152278_109631 , doesnt work right if run from an interactive node on hpc, since SLURM_JOB_ID is always set in that case...
    ...
    clear; sbatch -o ${log_dir}/RFAll_obs.out $calibrate_script  ${this_config_dir}/config_calibrate_RFAll_obs.jl
else
