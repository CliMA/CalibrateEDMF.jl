...
adapt_dt_list = repeat([true], length(dt_min_list))
dt_setups = zip(dt_min_list, dt_max_list, adapt_dt_list)
run_calibration_vars_list = [["ql_mean", "qi_mean", "qr_mean", "qip_mean"]]
# ============================================================== #
    ...
    for (experiment, setup) in run_setups
        for (dt_min, dt_max, adapt_dt) in dt_setups
            ...
            for calibration_vars in run_calibration_vars_list
                ...
                run(`cd $postprocessing_experiment_dir`)
                cd(postprocessing_experiment_dir)
                ...
                postprocessing_experiment_dir = joinpath(
                    postprocess_runs_storage_dir,
                    "subexperiments",
                    experiment,
                    "Calibrate_and_Run",
                    setup,
                    ...
                    calibration_vars_str,
                    "postprocessing",
                ) # `run` is kind of a catchall for any runs we do, so we want our own specific dir for post processing probably...
                mkpath(postprocessing_experiment_dir)
                # make sure the data storag dir linked to exists
                postprocessing_experiment_data_storage_dir = joinpath(
                    postprocess_runs_storage_data_dir,
                    "subexperiments",
                    experiment,
                    "Calibrate_and_Run",
                    setup,
                ...
                postprocessing_experiment_data_storage_dir_linked = joinpath(
                    postprocess_runs_storage_dir,
                    "subexperiments",
                    experiment,
                    "Data_Storage",
                ...
                run(
                    `ln -sf ../../../../../Data_Storage/Calibrate_and_Run/$setup/$dt_string/$calibration_vars_str/postprocessing/output  $postprocessing_experiment_dir/output`,
                ) # could also use  postprocessing_experiment_data_storage_dir_linked
                run(`cd $working_dir`)
                cd(working_dir)
                # --------------------------------------------------- #
        ...
        for (dt_min, dt_max, adapt_dt) in dt_setups
            ...
            for calibration_vars in run_calibration_vars_list
                ...
                CEDMF_output_dir = joinpath(
                    ...
                    "Atlas_LES",
                    "RFAll_obs",
                ) # the dir where the Diagnostics output is
                ...
                postprocess_output_dir = joinpath(
                    postprocess_runs_storage_dir,
                    "subexperiments",
                    experiment,
                    "Calibrate_and_Run",
                    ...
                    "Atlas_LES",
                    "RFAll_obs",
                ) # the dir where the postprocessed output is, is outside SOCRATES for rsync efficiency
                save_dir = joinpath(
                    postprocess_dir,
                    "subexperiments",
                    experiment,
                    ...
                    "Atlas_LES",
                    "RFAll_obs",
                ) # the dir where the collated output will be saved
                ...
                collate_postprocess_runs_slurm(
                    CEDMF_output_dir,
                    postprocess_output_dir,
                    save_dir;
                    overwrite = true,
                    overwrite_reference = reprocess_reference,
                    delete_TC_output_files = delete_TC_output_files,
                ) # this is kinda slow so maybe we could launch a slurm job for each one? idk...
