...
("qc_mean", "profiles") => x-> Dict(
    "data" => (filename, config, reference_model, z_scm)  -> get_reference_period_TC_output(filename, ["ql_mean"], config, reference_model; z_scm=z_scm) .+ get_reference_period_TC_output(filename, ["qi_mean"], config, reference_model; z_scm=z_scm), # repeated calls to get_profile are slow so maybe we can optimize this...
    "dimnames" => NC.dimnames(x.group["profiles"]["ql_mean"]),
    "attrib" => nothing # drop
    ),
)
...
function postprocess_run(
    ...
    methods = [method == "best_ensemble_mean" ? collect("best_ensemble_mean_{$i}" for i in 1:ensemble_size) : method for method in methods]
    ...
    for (i_m, method) in enumerate(methods)
        ...
        mkpath(log_dir)
        mkpath(data_dir)
        run(`sbatch  -o $log_dir/%x.out  --parsable --partition=expansion --kill-on-invalid-dep=yes -n $n_proc_scm $thisdir/postprocessor.sbatch $CEDMF_output_dir $data_dir $optimal_params_save_path $method` ) # no need for array yet
...
function collate_postprocess_runs(
    ...
    kwargs_ref_model = @invokelatest CalibrateEDMF.ReferenceModels.get_ref_model_kwargs(ref_config; global_namelist_args = namelist_args)
    ...
    if overwrite_reference
        ...
        @warn("Assuming all LES derived data are alredy in the reference files, overwrite_reference is false, so they will not be calculated")
    end
    ...
    all_data_vars::NTuple{nvars, Tuple{Tuple{String,String}, String}} = ( [(data_var, "data_vars") for data_var in data_vars]..., [(data_var, "derived_data_vars_LES") for data_var in keys(derived_data_vars_LES)]..., [(data_var, "derived_data_vars_TC") for data_var in keys(derived_data_vars_TC)]...)
                        ...
                        # @showprogress for (i_m, method) in enumerate(original_methods) # version w/ progress meter
                            if method ∉ methods_ensemble # these don't use the particle dimension
                                if (flight_number, forcing_type) ∉ keys(out_files[method])
                                    ...
                                    if (varsource == "data_vars") ||  (varsource == "derived_data_vars_TC") || (varsource == "derived_data_vars_LES")
                                ...
                                # @showprogress for particle in coords["particle"]
                                    ...
                                    if (flight_number, forcing_type) ∉ keys(out_files[particle_method])
                                        ...
                                        if (varsource == "data_vars") || (varsource == "derived_data_vars_LES") # derived_data_vars_TC we'll do later from full arrays, derived_data_vars_LES ones are already there
                                            ...
                                            @inbounds _new_data_ens[:, i_fn:i_fn, i_c:i_c, particle:particle] .= var_data 
                                        end

                                        if (varsource == "data_vars") ||  (varsource == "derived_data_vars_TC")  || (varsource == "derived_data_vars_LES")
                                            @inbounds _new_data_ens[:, i_fn:i_fn, i_r:i_r, particle:particle] .= truth_data
                                        end
                                    end
                                end
                                ...
                                @inbounds _new_data[:, :, :, i_m:i_m] .= @invokelatest reducer(_new_data_ens[:,:,:,:], dims=4) # not sure why this breaks even w/ safe_include but oh well...
                            end
                        end
...
function collate_postprocess_runs_slurm(
    ...
    run(`sbatch  -o $save_dir/collated_data/%x.out  --parsable --partition=expansion --kill-on-invalid-dep=yes $thisdir/collate_postprocess_runs.sbatch $CEDMF_output_dir $tc_output_dir $save_dir $overwrite $overwrite_reference $reference_files_already_have_derived_data_vars_LES $delete_TC_output_files` ) # no need for array yet
