...
# SAVED_WEIGHTS_PATH = <PATH_TO_NN_PRETRAINED_WEIGHTS_JLD2>


f = jldopen(SAVED_WEIGHTS_PATH, "r+")
WEIGHTS_SERIALIZED = f["serialized_weights"][:]
WEIGHTS_SERIALIZED = convert(Vector{Float64}, WEIGHTS_SERIALIZED)


function replace_params!(
    optimal_params_u_name::Vector{String},
    optimal_params_u::Vector{Any},
    new_params::Vector{Float64},
)
    # Find the index of "linear_ent_params" in the names vector
    index = findfirst(isequal("linear_ent_params"), optimal_params_u_name)

    if index !== nothing
        # Replace the parameter name
        optimal_params_u_name[index] = "nn_ent_params"

        # Replace the parameter vector
        optimal_params_u[index] = new_params
    end
end
...
namelist_args = [
    ...
    ("turbulence", "EDMF_PrognosticTKE", "nn_arc", NN_ARC),
...
# replace linreg params with nn params
replace_params!(optimal_params_u_name, optimal_params_u, WEIGHTS_SERIALIZED)
...
function get_prior_config()
    ...
    config["unconstrained_σ"] = Dict(
        ...
        "nn_ent_params" => repeat([0.15], NUM_NN_PARAMS),
