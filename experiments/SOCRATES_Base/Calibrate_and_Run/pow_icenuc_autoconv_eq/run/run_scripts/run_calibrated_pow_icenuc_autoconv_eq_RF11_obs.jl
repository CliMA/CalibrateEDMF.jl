...
calibrated_config_dir = "RF11_obs" # 12 obs
...
# /net/sampo/data1/jbenjami/Research_Schneider/CliMa/CalibrateEDMF.jl/experiments/SOCRATES_Test_Dynamical_Calibration/Calibrate_and_Run/pow_icenuc_autoconv_eq/calibrate/output/RF01_obs/Diagnostics.nc
...
for (i,parameters) in enumerate(parameters_list) # store things so we can iterate jobs regardless of which parameters dict theyre from, so that we can keep all workings going all the time (since the parameters dicts ave different numbers of jobs)
    ...
    cp(@__FILE__, joinpath(output_dir, "run_calibrated_pow_icenuc_autoconv_eq_RF11_obs.jl"), force=true)
