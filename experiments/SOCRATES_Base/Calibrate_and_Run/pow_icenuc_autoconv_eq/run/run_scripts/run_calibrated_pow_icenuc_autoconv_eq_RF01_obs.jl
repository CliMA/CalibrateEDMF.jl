...
calibrated_config_dir = "RF01_obs"
calibrated_config_path = joinpath(experiment_dir, "Calibrate_and_Run", "pow_icenuc_autoconv_eq", "calibrate", "output", calibrated_config_dir)
@info("here", calibrated_config_path)
# /net/sampo/data1/jbenjami/Research_Schneider/CliMa/CalibrateEDMF.jl/experiments/SOCRATES_Test_Dynamical_Calibration/Calibrate_and_Run/pow_icenuc_autoconv_eq/calibrate/output/RF01_obs/Diagnostics.nc # example path
...
is_valid_setup = setup -> begin  # filter out if datafile doesn't exist (11 :obsdata for example) (logger seems unable to handle failures that arent return codes i guess...)
...
case_names = [ "SOCRATES_RF"*string(setup["flight_number"],pad=2)*"_"*string(setup["forcing_type"]) for setup in setups]
...
# ))
# push!(parameters_list, Dict( # noneq obs
#     "case_name" =>  filter(contains("obs"),case_names),
...
datetime_suffixes = [string(forcing_types[contains.(parameters["case_name"][1], string.(forcing_types))][]) * "__" * parameters["moisture_model"][] for parameters in parameters_list] # works with our current implementation
...
for (i,parameters) in enumerate(parameters_list) # store things so we can iterate jobs regardless of which parameters dict theyre from, so that we can keep all workings going all the time (since the parameters dicts ave different numbers of jobs)
    ...
    output_dir = joinpath(experiment_dir, "Calibrate_and_Run", "pow_icenuc_autoconv_eq", "run", "output", calibrated_config_dir, datetime_suffixes[i])  # drop the datetime info, just keep the file at the suffix so if we run again it just overwrites...
    mkpath(output_dir) # make it now so we only have to do it once...
    cp(@__FILE__, joinpath(output_dir, "run_calibrated_pow_icenuc_autoconv_eq_RF01_obs.jl"), force=true)
    ...
    descriptors[i] = [rstrip(join([string(key)*"-"*string(val)*"__" for (key,val) in filter( p -> p[1] in name_heirarchy[0], sorted_setup) ]), '_') for sorted_setup in sorted_setups[i]]
    names[i]      = [rstrip(join([string(key)*"-"*string(round(log10(val),sigdigits=4))*"__" for (key,val) in filter( p -> p[1] in name_heirarchy[1] , sorted_setup) ]), '_')  for sorted_setup in sorted_setups[i]]# we don't have a dict w/ user_args, user_aux so this should be simpler too i think... copy same code from descriptor
    ...
    out_datas[i] = Dict(
        ...
        "tau" => Dict(
            "liq" => parameters["τ_cond_evap"],
            "ice" => parameters["τ_sub_dep"],
        )
    ) # a dict copying format from TrainTau.jl, used to label the output data after process_sensitivity_set
    out_datas[i]["dir_array"] = reshape(unique(names[i]), length(parameters["τ_cond_evap"]), length(parameters["τ_sub_dep"]) ) # unique cause could be repeated from different cases and such... techincally though this only works if everythin else only has a single option like we did in TrainTau.jl too....
...
# for parameters in parameters_list # iterate over our simulations for now instead of merging them... if we merge them but want separation we could look into ways to keep their timestamps separate...
    ...
    for (setup,output_dir) in zip(setups_list, setups_output_dirs_list)
            ...
            if pName != "case_name" # don't need to overwirte the case_name since the namelist was generated for them
                ...
                try
                    ...
                    if pName in keys(alt_param_placements)
                        @info("using known alternate paramter placement in namelist subdict " * string(alt_param_placements[pName]) * " for parameter " * pName)
                    else
                        error("key $pName not found in namelist and no alternate placement specified in alt_param_placements")
                    end
        ...
        descriptor = rstrip(join([string(key)*"-"*string(val)*"__" for (key,val) in filter( p -> p[1] in name_heirarchy[0] , sorted_setup) ]), '_')
        name       = rstrip(join([string(key)*"-"*string(round(log10(val),sigdigits=4))*"__" for (key,val) in filter( p -> p[1] in name_heirarchy[1] , sorted_setup) ]), '_') # we don't have a dict w/ user_args, user_aux so this should be simpler too i think... copy same code from descriptor
