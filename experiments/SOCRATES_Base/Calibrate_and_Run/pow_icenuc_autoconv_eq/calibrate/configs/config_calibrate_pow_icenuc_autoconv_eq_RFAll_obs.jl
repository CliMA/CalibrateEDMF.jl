...
# _, trimmed_reference_paths = process_SOCRATES_Flight_Observations_reference(out_vars=calibration_vars) # create trimmed data for calibration so the covarainces aren't NaN (failed anyway cause i think after interpolation NaNs can return because the non-NaN data don't form a contiguous rectangle :/ )
...
if calibrate_to == "Flight_Observations"
    if !ismissing(t_bnds.obs_data)
        ...
        _, obs_paths = process_SOCRATES_Flight_Observations_reference(out_vars=calibration_vars, fake_time_bnds = t_bnds.obs_data) # create trimmed data for calibration so the covarainces aren't NaN (failed anyway cause i think after interpolation NaNs can return because the non-NaN data don't form a contiguous rectangle :/ )
    end
    if !ismissing(t_bnds.ERA5_data)
        @info("Creating trimmed observational ERA5_data-forced data for calibration")
        _, ERA5_paths = process_SOCRATES_Flight_Observations_reference(out_vars=calibration_vars, fake_time_bnds = t_bnds.ERA5_data) # create trimmed data for calibration so the covarainces aren't NaN (failed anyway cause i think after interpolation NaNs can return because the non-NaN data don't form a contiguous rectangle :/ )
    end
    # concatenate
    reference_paths = [obs_paths; ERA5_paths]
else
    reference_paths = Dict() # for regular atlas_les, keep empty, we'll just generate them later
end
...
function get_output_config()
    config = Dict()
    config["outdir_root"] = joinpath(experiment_dir, "Calibrate_and_Run","pow_icenuc_autoconv_eq", "calibrate", "output", calibrate_to, "RFAll_obs") # store them in the experiment folder here by default
...
function get_reference_config(::SOCRATES_Val)
    ...
    for setup in setups
       name = "RF"*string(setup["flight_number"],pad=2)*"_"*string(setup["forcing_type"])
       datafile = joinpath(truth_dir, name, "stats", name*".nc")
       if isfile(datafile) # I think we gotta be careful about this cause if we're using flight obs, 11 obs exists but then the simulation won't run...
           setup["datafile"] = datafile
           setup["case_name"] = "SOCRATES_"*name
       else
           @warn("File $datafile does not exist")
    ...
    # config["time_shift"] = config["time_shift"][] # test to see if vector here was the problem
    # config["batch_size"] = n_repeat # has to be some divisor of n_repeat, default is n_repeat == length(ref_dirs) == number of setups
    local_namelist = append!(local_namelist, [("thermodynamics", "moisture_model", "nonequilibrium")]) # TC.jl || overwrite_namelist | # list of tuples (<namelist_section>, <namelist_key>, <value>) matching namelist[<namelist_section>][<namelist_key>] = <value>
