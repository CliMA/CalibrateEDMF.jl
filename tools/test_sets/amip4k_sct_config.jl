...
# Get all shallow AMIP4K cases from Shen et al (2022)
les_library = get_shallow_LES_library()

struct ScmTest end
struct Amip4K_SCT end
namelist_args = [
...
function get_reference_config(::Amip4K_SCT)
    config = Dict()
    ...
    for model in keys(les_library)
        for month in keys(les_library[model])
            ...
            les_kwargs = (forcing_model = model, month = parse(Int, month), experiment = "amip4K")
