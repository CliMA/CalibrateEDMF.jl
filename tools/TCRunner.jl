...
struct TrainingSet end
struct ValidationSet end
struct TestSet end
...
"""
    run_TC_optimal(results_dir::String, output_dir::String, config::Dict; method::String = "best_nn_particle_mean", metric::String = "mse_full" )
...
function run_TC_optimal(
    ...
    for i in 1:length(run_cases["case_name"])
        run_SCM_handler(
            ...
            les = get_stats_path(run_cases["y_dir"][i]),
        )
    end
end
