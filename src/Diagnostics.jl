...
function io_dictionary_val_reference()
    io_dict = Dict(
        ...
        "ref_variable_names_val" => (; dims = ("config_val", "config_field_val"), group = "reference", type = String),
    )
    return io_dict
end
function io_dictionary_val_reference(
...
function io_dictionary_val_particle_eval()
    io_dict = Dict(
        ...
        "val_mse_by_var_full" => (; dims = ("particle", "config_field", "iteration"), group = "particle_diags", type = Float64),
    )
    return io_dict
end
function io_dictionary_val_particle_eval(
