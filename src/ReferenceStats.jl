...
- `tikhonov_mode`    :: If "relative", tikhonov_noise is scaled by the maximum eigenvalue in the covariance matrix 
    considered, having the interpretation of the inverse of the desired condition number. This value is enforced to be
...
- `model_errors`     :: Vector of model errors added to the internal variability noise, each containing the model error
    per variable normalized by the pooled variable variance.
"""
...
Typically, the observations are fetched by specifying `y_type` and `Σ_type`, which indicate if the data is generated 
by the [`SCM`](@ref) or [`LES`](@ref). 
...
The keyword `normalize` specifies whether observations are to be normalized with respect to the per-quantity pooled 
variance or not. See [`normalize_profile`](@ref) for details.
...
- `model_error`  :: Model error per variable, added to the internal variability noise, and normalized by the pooled 
    variance of the variable.

# Returns
`Matrix`    :: Covariance of data (columns of variables) with samples across the time dimension.
`Vector`    :: Vertically averaged time-variance for each variable (length = number of variables)
"""
