...
Base.@kwdef struct ReferenceStatistics{FT <: Real, IT <: Integer}
    ...
    function ReferenceStatistics(
        ...
        for (i, m) in enumerate(RM)
            ...
            y_, y_var_, pool_var = get_obs(model, normalize, z_scm = get_z_obs(model), model_error = model_error, obs_var_scaling = obs_var_scaling)
...
function get_obs(
    ...
    # time covariance
    Σ, pool_var = get_time_covariance(m, Σ_names, z_scm, normalize = normalize, model_error = model_error, obs_var_scaling = obs_var_scaling)
    # normalization
